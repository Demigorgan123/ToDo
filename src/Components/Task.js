...
function Task({ title, date, description, completed }) {
        ...
        for(let i = 0; i<tempTasks.length; i++){
            if(tempTasks[i].title === title){
                if (tempTasks[i].completed) {
                    tempTasks[i].completed = 'Completed';
                }
            }
        }
    ...
    return (
        <div className={`p-[20px] bg-slate-800 rounded-md ${completed === "Completed"?"hidden":""}`}>
            ...
            <section className='flex gap-[20px]'>
                ...
                <button className={`bg-slate-600 py-[10px] px-[20px] text-slate-100 text-sm rounded-md`} onClick={markAsComplete}>Mark as complete</button>
                <div className={`bg-green-600 py-[10px] px-[20px] text-slate-100 text-sm rounded-md ${hide === "N"?"hidden":""}`}>Completed</div>
            </section>
        </div>
    )
}
